...
Define the CLI interface for parallel CPU-core configuration during document ingestion, enabling users to control the
number of worker processes for multicore document processing.

## Requirements

### Requirement: CLI supports core count configuration

The system SHALL provide a `--cores` / `-c` CLI option for the `ingest` command that specifies the number of CPU cores
...
The system SHALL use `ProcessPoolExecutor` to extract text from multiple documents in parallel using the configured
number of cores.
...
The system SHALL use `ProcessPoolExecutor` to chunk extracted text from multiple documents in parallel using the
